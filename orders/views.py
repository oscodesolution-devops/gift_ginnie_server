...
class CouponView(APIView):
    ...
    def get(self, request):
        try:
            ...
            return Response({"message": "Coupons fetched successfully.", "data": serializer.data}, status=status.HTTP_200_OK)
...
class CartView(APIView):
    ...
    def get(self, request):
        try:
            ...
            return Response({"message": "Cart fetched successfully.", "data": serializer.data}, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({"message": f"Error occurred while fetching cart {e}"}, status=status.HTTP_400_BAD_REQUEST)
...
class ApplyCouponView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if coupon.valid_from > now() or coupon.valid_until < now():
                return Response({"message": "This coupon is expired."}, status=status.HTTP_400_BAD_REQUEST)
            ...
            if (coupon.max_usage_per_user and user_usage_count >= coupon.max_usage_per_user):
                return Response({"message": f"You have reached your usage limit for this coupon ({coupon.max_usage_per_user})."}, status=status.HTTP_400_BAD_REQUEST)
            if cart.items.count() == 0:
                return Response({"message": "Cart is empty, add items before applying coupon."}, status=status.HTTP_400_BAD_REQUEST)
            usage = CouponUsage.objects.filter(user=request.user, coupon=coupon).first()
            if usage:
                return Response({"message": "You cannot apply the same coupon twice."}, status=status.HTTP_400_BAD_REQUEST)
            cart.coupon = coupon
            cart.save()

            return Response({"message": "Coupon applied successfully.", "data": {"coupon": coupon.code}}, status=status.HTTP_200_OK)
        except Exception as e:
    ...
    def delete(self, request):
        ...
        try:
            ...
            return Response({"message": "Coupon removed from the cart successfully."}, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({"message": str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class CartItemView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            existing_item = CartItem.objects.filter(cart=user_cart, product=data.get("product_id")).first()
            ...
            if serializer.is_valid():
                serializer.save()
                return Response({"message": "Item added to cart successfully.", "data": serializer.data}, status=status.HTTP_201_CREATED)
            return Response({"message": "Failed to add item to cart.", "errors": serializer.errors}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            return Response({"message": f"Error occurred while adding item to cart {e}"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def patch(self, request, cart_item_id):
        ...
        try:
            ...
            return Response({"message": "Failed to update cart item.", "errors": serializer.errors}, status=status.HTTP_400_BAD_REQUEST)
        except CartItem.DoesNotExist:
            return Response({"message": "Cart item does not exist."}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            return Response({"message": "Failed to update cart item.", "errors": e.args}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def delete(self, request, cart_item_id):
        try:
            ...
            if cart_item:
                ...
                return Response({"message": "Cart item doeys not exist."}, status=status.HTTP_404_NOT_FOUND)
        except CartItem.DoesNotExist:
            return Response({"message": "Cart item does not exist."}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            return Response({"message": "Failed to update cart item.", "errors": e.args}, status=status.HTTP_400_BAD_REQUEST)
...
class CheckoutView(APIView):
    ...
    def get(self, request):
        try:
            ...
            return Response({"message": f"Error occurred while fetching orders {e}"}, status=status.HTTP_400_BAD_REQUEST)

    def post(self, request):
        address_id = request.data.get("address_id")
        if not address_id:
            return Response({"message": "Address ID not provided.", "data": ["address_id is required"]}, status=status.HTTP_400_BAD_REQUEST)
        try:
        ...
        if user_cart.items.count() == 0:
            return Response({"message": "Cart is empty, add items before checkout."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            with transaction.atomic():
                ...
                order = Order.objects.create(user=request.user, total_price=total_price, status="PENDING", delivery_address=address, final_price=final_price, discount_applied=discount_applied)
                # Add items to the order
                for item in user_cart.items.all():
                    if not item.product.in_stock():
                        raise ValueError("Some products are out of stock, please update your cart.")
                    order_item = OrderItem.objects.create(order=order, product=item.product, quantity=item.quantity, price=item.price)
                    order_item.save()
                    item.product.save()

                order.save()
                # Create a Razorpay Order
                razorpay_order = client.order.create({"amount": int(final_price * 100), "currency": "INR", "receipt": f"order_rcptid_{order.id}", "payment_capture": "1"})
                # Attach Razorpay Order ID to the local order
                order.razorpay_order_id = razorpay_order["id"]
                order.save()

            return Response({"message": "Order created successfully.", "order_id": order.id, "razorpay_order_id": razorpay_order["id"], "amount": final_price, "currency": "INR"}, status=status.HTTP_201_CREATED)

        except ValueError as e:
            return Response({"message": "Some products are out of stock, please update your cart."}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            return Response({"message": str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class VerifyPaymentView(APIView):
    ...
    def post(self, request):
        try:
            ...
            return Response({"success": True, "message": "Payment verified successfully.", "data": serializer.data}, status=status.HTTP_200_OK)
...
def razorpay_webhook(request):
    if request.method == "POST":
        try:
            ...
            client = razorpay.Client(auth=(settings.RAZORPAY_API_KEY, settings.RAZORPAY_API_SECRET))
            client.utility.verify_webhook_signature(payload, received_signature, "hello@123")
